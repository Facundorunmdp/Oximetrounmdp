...
-------------------------------------------------------------------------------
        ...
        self.timer1.setInterval(400) #250 s/seg son 400 ms
                                    #500 s/seg  200 ms                                
                                    #1ks/seg    100 ms
...
----------------Accion de los Botones------------------------------------------
        ...
        self.btnConectar.clicked.connect(self.conectar_handler)     
        self.pblimpiarpantalla.clicked.connect(self.limpiar_pantalla)
        self.pbinfo.clicked.connect(self.info)        
            
        #---pestaña Configuracion--------------------
             #----pestaña diagnosticar----------------------------------
        self.btnResetAFE.clicked.connect(self.resetAFE)
        self.btnDiagnosticar.clicked.connect(self.diagnosticar)
            #----pestaña Etapa TX----------------------------------
        """accion de la señal de los Slider:
            escribir el valor de corriente en lblCurrent2 y lblCurrent1"""
        self.Slider1.valueChanged.connect(self.modificarlbl1)
        ...
        self.Slider3.valueChanged.connect(self.modificarlbl3)
        self.btnSetearRx.clicked.connect(self.setearRx)   
        ...
        self.btnSetearTiempos.clicked.connect(self.setearTiempos)    
        
                    
        #----------------------pestaña guardar---------------------------------
        self.pbguardarprueba.clicked.connect(self.guardar_prueba)
        ...
        self.gvtemporal.setTitle("Grafica en tiempo real")
    ...
    def cargar_datos(self):
        print("cargar datos")
        self.archivo_offline, _ = QFileDialog.getOpenFileName(self, 'Abrir archivo', 'C:/Users/Windows 10/Documents/Tesis 2020/archivosproyectopython', 'Text files (*.csv)')
    ...
    def Datos(self):
        ...
        """
        self.dataY = np.zeros((self.numero_canales,10000,)) #creamos un np array de 4 elementos donde se iran agregando los datos recibidos y formateados a 24 bits
    ...
    def procesar_datos(self,_NIVEL_BATERIA,_datos):
        ...
        #limitamos la matriz a 10.000 muestras
        if self.dataY.shape[1] >= 10100:
            self.dataY = self.dataY[:,-10000:]
...
class External(QThread): #se crea la clase External que hereda de QThread
    ...
    que hay daos listos para procesar y llamara a la funcion "procesar_datos"
    """
    def run(self):
        """
        ...
        while (main.comando == 'datos'):
            ...
            if (recibido[0]== mask and len(recibido)==603) :
                ...
                datos=[] #paquete de 100 muestras de 4 canales en formato de 24bits
